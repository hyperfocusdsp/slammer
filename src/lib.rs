//! Niner — kick drum synthesizer plugin.
//!
//! This crate exposes Niner as a VST3 + CLAP plugin (via `cdylib`) and as
//! a standalone binary (`niner-standalone`). The module tree is declared
...
pub use plugin::Niner;

nih_export_vst3!(plugin::Niner);
nih_export_clap!(plugin::Niner);
...
pub fn run_standalone() {
    ...
    logging::init();
    tracing::info!(
        "Niner standalone v{} starting (os={}, arch={})",
    ...
    #[cfg(target_os = "windows")]
    {
        if let Some(argv) = windows_standalone::probed_argv() {
            nih_export_standalone_with_args::<plugin::Niner, _>(argv);
            return;
        }
    }

    nih_export_standalone::<plugin::Niner>();
}
